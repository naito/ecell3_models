...
LIBECS_DM_CLASS( MembraneAssignmentProcess, Process )
{
 ...
 public:
	...
	virtual void fire()
	{
		...
		_Nae_MolarConc = Nae->getMolarConc();
		_Nai_MolarConc = Nai->getMolarConc();

		_Cae_MolarConc = Cae->getMolarConc();
		_Cai_MolarConc = Cai->getMolarConc();

		EK->setValue( _R_FzK * _T * log( _Ke_MolarConc / _Ki_MolarConc ) );
		ENa->setValue( _R_FzNa * _T * log( _Nae_MolarConc / _Nai_MolarConc ) );
		ECa->setValue( _R_FzCa * _T * log( _Cae_MolarConc / _Cai_MolarConc ) );

...
V_tissue = 5.25     # cm^3
V_blood  = 1.03     # cm^3
F_perfusate = 50    # ml/min
V_perfusate = 100   # cm^3
...
FFA_Tissue  = 0.71   # umol/gww hep (not sure)
...
System System( /TISSUE )
{
	...
	{
		MolarConc	0.00149685660114;
		Fixed	0;
	}
	
	Variable Variable( NADH )
	{
		MolarConc	3.14339886239e-06;
		Fixed	0;
	}
...
omega_PC = "( 1.0 / ( 1.0 + pow( %s, n3 ) / Ka_AcCoA ) + K_ATP / Ki_ADP_ATP * %s / pow( %s, n1 ) )" % ( C( 'AcCoA' ), C( 'ADP' ), C( 'ATP' ) )
	...
	Process ExpressionFluxProcess( R_FA_endo ) @# Table S1
	{
		VariableReferenceList
			[ time :/:time 0 ]
			[ FFA :.:FFA 1 ];
	...
	#	Expression "time.Value";
		...
		Expression "((leq( time.Value, 30.0 )*0.105) + (and( gt( time.Value, 30.0 ), leq( time.Value, 90.0 )) * 0.0573)) * 1.0e-3 * self.getSuperSystem().SizeN_A";  @# umol/gww hep/min
...
System System( /BLOOD )
{
	...
	Process ExpressionFluxProcess( J_FA_bt_net ) @# Table S1
	{
	#	StepperID PSV;
	...
	#	Expression "time.Value";	
		...
		Expression  "and( gt( time.Value, 90.0 ), leq( time.Value, 120.0 )) * 0.0573 + 0.27 * (1-exp(-(time.Value - 60.0) / tao)) * 1.0e-3 * self.getSuperSystem().SizeN_A";
...
System System( /PERFUSATE )
{
	...
	Variable Variable( SIZE )
	{
		Value 	@( V_blood/1000.0);  @# cm^3 → L  #1.0; @# とりあえずの値 論文の値に修正すること
	}
...
#	}
	...
	##### Number of Molecules Perfusate --> Blood #####

#		Process ExpressionFluxProcess( GLC_Perfusate_Blood )
#	{
#		VariableReferenceList
#			[ p :/PERFUSATE:GLC -1 ]
#			[ b :/BLOOD:GLC      1 ];
#
#		Expression "(@C( 'p' ) - @C( 'b' ))*1.0e-3*self.getSuperSystem()#.SizeN_A";
#
#	}

	Process ExpressionFluxProcess( PYR_Perfusate_Blood )
	{
		VariableReferenceList
			[ time :/:time 0 ]
			[ p :/PERFUSATE:PYR -1 ]
	...
	Expression "and( gt( time.Value, 30.0 ), leq( time.Value, 120.0 )) * (@C( 'p' ) - @C( 'b' ))*1.0e-3*self.getSuperSystem().SizeN_A";

	}

	Process ExpressionAssignmentProcess( PYR_Perfusate )
	{
		StepperID PSV;
	...
	#	Expression "and( gt( time.Value, 30.0 ), leq( time.Value, 120.0 )) * 2 * (1-exp(-(time.Value - 30.0)/ tao)) * 1.0e-3*self.getSuperSystem().SizeN_A";
			

		Expression "and( gt( time.Value, 30.0 ), leq( time.Value, 120.0 )) * 2 * (1-exp(-(time.Value - 30.0)/ tao))";

		tao   3.0; # min

	}

	Process ExpressionFluxProcess( LAC_Perfusate_Blood )
	{
		VariableReferenceList
			[ p :/PERFUSATE:LAC -1 ]
			[ b :/BLOOD:LAC      1 ];
	...
	Process ExpressionFluxProcess( AcAc_Perfusate_Blood )
	{
		VariableReferenceList
			[ p :/PERFUSATE:AcAc -1 ]
			[ b :/BLOOD:AcAc      1 ];
	...
	Process ExpressionFluxProcess( BHB_Perfusate_Blood )
	{ 
		VariableReferenceList
			[ p :/PERFUSATE:BHB -1 ]
			[ b :/BLOOD:BHB      1 ];
	...
	Process ExpressionFluxProcess( FFA_Perfusate_Blood )
	{
		VariableReferenceList
			[ p :/PERFUSATE:FFA -1 ]
			[ b :/BLOOD:FFA      1 ];
	...
	Process ExpressionFluxProcess( ALA_Perfusate_Blood )
	{
		VariableReferenceList
			[ p :/PERFUSATE:ALA -1 ]
			[ b :/BLOOD:ALA      1 ];
	...
	Process ExpressionFluxProcess( GLR_Perfusate_Blood )
	{
		VariableReferenceList
			[ p :/PERFUSATE:GLR -1 ]
			[ b :/BLOOD:GLR      1 ];
	...
	Process ExpressionFluxProcess( TG_Perfusate_Blood )
	{
		VariableReferenceList
			[ p :/PERFUSATE:GLR -1 ]
			[ b :/BLOOD:GLR      1 ];

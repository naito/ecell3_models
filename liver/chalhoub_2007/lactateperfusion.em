...
omega_PC = "( 1.0 / ( 1.0 + pow( %s, n3 ) / Ka_AcCoA ) + K_ATP / Ki_ADP_ATP * %s / pow( %s, n1 ) )" % ( C( 'AcCoA' ), C( 'ADP' ), C( 'ATP' ) )
	...
	Process ExpressionFluxProcess( R_FA_endo ) @# Table S1
	{
		...
		Expression "(leq( time.Value, 30.0 )*0.105) + (and( gt( time.Value, 30.0 ), leq( time.Value, 60.0 )) * 0.0573) * 1.0e-3 * self.getSuperSystem().SizeN_A";  @# umol/gww hep/min
...
System System( /BLOOD )
{
	...
	Process ExpressionAssignmentProcess( J_FA_bt_net ) @# Table S1
	{
		StepperID PSV;
		...
		Expression  "and( gt( time.Value, 60.0 ), leq( time.Value, 90.0 )) * 0.0573 + 0.27 * (1-exp(-(time.Value - 60.0) / tao)) * 1.0e-3 * self.getSuperSystem().SizeN_A";
...
#	}
	...
	Process ExpressionFluxProcess( LAC_Perfusate )
	{
		VariableReferenceList
			[ time :/:time 0 ]
			[ p :/PERFUSATE:LAC -1 ]
		...
		Expression  "and( gt( time.Value, 30.0 ), leq( time.Value, 90.0 )) * (@C( 'p' ) - @C( 'b' ))*1.0e-3*self.getSuperSystem().SizeN_A";
	}
	
	Process ExpressionAssignmentProcess( LAC_Perfusate_Blood )
	{
		StepperID PSV;
		...
		Expression  "and( gt( time.Value, 30.0 ), leq( time.Value, 90.0 )) * 10 * (1-exp(-(time.Value-30)/tao))";
